...
function dniLetter( dni ) {
	...
	var dividedBy23 = dni % 23;
	var lockup = 'TRWAGMYFPDXBNJZSQVHLCKE';
	var result = lockup.charAt(dividedBy23);
...
function findLongestWord( array_words ){
	...
	for (i in array_words){
		if(max_length < array_words[i].length){
			max_length = array_words[i].length;
		}
	}
	return max_length;
...
array_lengths.push( findLongestWord( ["Yellow", "Blue", "Purple"] ) );
array_lengths.push( findLongestWord( ["Hippo", "Elephant", "Rhino", "Giraffe" ] ) );
array_lengths.push( findLongestWord( ["Happy", "Dopey", "Bashful"] ) );
array_lengths.push( findLongestWord( ["Bookkeeper", "Doctor", "Cook"] ) );
